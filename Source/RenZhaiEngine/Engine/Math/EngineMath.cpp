...
namespace EngineMath
{
    ...
    bool IsInTheScope(const fvector_2d& InStartPosition, const fvector_2d& InEndPosition, const fvector_2d& InCurrentPosition)
    {
        if (InStartPosition.x <= InCurrentPosition.x && InStartPosition.y <= InCurrentPosition.y &&
            InCurrentPosition.x <= InEndPosition.x && InCurrentPosition.y <= InEndPosition.y)
        {
            return true;
        }
